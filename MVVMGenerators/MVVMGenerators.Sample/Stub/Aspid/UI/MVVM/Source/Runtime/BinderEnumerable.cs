...
{
    public sealed class BinderEnumerable<T, TBinder> : Binder, IBinder<T>, IViewModel
        where TBinder : IBinder<T>
    {
        private event Action<T>? Changed;
        
        private readonly IEnumerable<TBinder> _binders;
        
        public bool IsReverseEnabled { get; private set; }

        public BinderEnumerable(IEnumerable<TBinder> binders)
        {
            _binders = binders;
        }
